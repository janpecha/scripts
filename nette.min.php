...
/**
 * Nette Framework (version 2.0-dev released on 2011-12-09, http://nette.org)
 *
...
class Application extends Nette\Object
{
	...
	function __construct(IPresenterFactory $presenterFactory, IRouter $router, Nette\Http\IRequest $httpRequest,
	...
	function match(Nette\Http\IRequest $httpRequest)
	{
		...
		foreach ($this->metadata as $name => $meta) {
			if (isset($params[$name])) {
				...
				} elseif (isset($meta[self::FILTER_TABLE])) {
...
abstract class PresenterComponent extends Nette\ComponentModel\Container implements ISignalReceiver, IStatePersistent, \ArrayAccess
{
	...
	final function getParameter($name = NULL, $default = NULL)
	{
		if (func_num_args() === 0) {
			return $this->params;
	...
	final function getParameterId($name)
	{
...
abstract class Presenter extends Control implements Application\IPresenter
{
	...
	final function popGlobalParameters($id)
	{
...
class PresenterComponentReflection extends Nette\Reflection\ClassType
{
	...
	function getPersistentComponents($class = NULL)
	{
		$class = $class === NULL ? $this->getName() : $class;
...
use Nette;use Nette\Caching\Cache;
...
class Configurator extends Nette\Object
{
	...
	function __construct()
	{
		...
		Nette\Environment::setConfigurator($this);
	}

	function setCacheDirectory($path)
	{
...
final class Environment
{
	...
	static function setConfigurator(Nette\Config\Configurator $configurator)
	{
		if (self::$createdAt) {
...
final class Framework
{
	...
	const NAME = 'Nette Framework',
		VERSION = '2.0-beta',
		REVISION = 'e318f79 released on 2011-12-09';
...
final class ObjectMixin
{
	...
	static function callProperty($_this, $name, $args)
	{
		if (strlen($name) > 3) {
			$op = substr($name, 0, 3);
...
use Nette;use Nette\Utils\Validators;
...
class Compiler extends Nette\Object
{
	...
	function addExtension($name, CompilerExtension $extension)
	{
	...
	function compile(array $config, $className)
	{
		$this->config = $config;
	...
	function generateCode($className)
	{
	...
	static function parseService(Nette\DI\ServiceDefinition $definition, $config, $shared = TRUE)
	{
		if (!is_array($config)) {
...
class Config extends Nette\Object
{
	...
	function load($file, $section = NULL)
	{
		if (!is_file($file) || !is_readable($file)) {
	...
	function addAdapter($extension, $adapter)
	{
	...
	static function takeParent(& $data)
	{
...
use Nette;use Nette\DI\ContainerBuilder;
...
class PhpExtension extends Nette\Config\CompilerExtension
{
	...
	function afterCompile(ContainerBuilder $container, Nette\Utils\PhpGenerator\ClassType $class)
	{
		...
		foreach ($this->options as $name => $value) {
			...
			} elseif (ini_get($name) != $value && !Nette\Framework::$iAmUsingBadHost) {
...
if (class_exists('PDO')){ class Connection extends PDO
{
	...
	function __construct($dsn, $username = NULL, $password  = NULL, array $options = NULL, IReflection $databaseReflection = NULL)
	{
...
class ConventionalReflection extends Nette\Object implements Nette\Database\IReflection
{
	...
	function setConnection(Nette\Database\Connection $connection)
	{}
...
class DiscoveredReflection extends Nette\Object implements Nette\Database\IReflection
{
	...
	function __construct(Nette\Caching\IStorage $storage = NULL, $foreign = '%s_id')
	{
	...
	function getPrimary($table)
	{
		...
		foreach ($query as $column) {
			if ($column[$primaryKey] === $primaryVal) {
				if ($primary !== NULL) {
					$primary = NULL;
					break;
				}
...
class Selection extends Nette\Object implements \Iterator, \ArrayAccess, \Countable
{
	...
	{
		return preg_replace('~(?:\\b[a-z_][a-z0-9_.:]*\.)?([a-z_][a-z0-9_]*)\.([a-z_*])~i', '\\1.\\2', $expression);
	}
...
class Container extends Nette\FreezableObject implements IContainer
{
	...
	function addService($name, $service, array $meta = NULL)
	{
		$this->updating();
	...
	function createInstance($class, array $args = array())
	{
...
use Nette\Utils\Validators;use Nette\Utils\PhpGenerator\Helpers as PhpHelpers;use Nette\Utils\PhpGenerator\PhpLiteral;
...
class ContainerBuilder extends Nette\Object
{
	...
	function autowireArguments($class, $method, array $arguments)
	{
		$rc = Nette\Reflection\ClassType::from($class);
		if (!$rc->hasMethod($method)) {
	...
	function getServiceName($arg, $self = NULL)
	{
		if (!preg_match('#^@\w+$#', $arg)) {
			return FALSE;
		}
...
final class Helpers
{
	...
	static function expand($var, array $params, $recursive = FALSE)
	{
		if (is_array($var)) {
			$res = array();
	...
	static function autowireArguments(\ReflectionFunctionAbstract $method, array $arguments, $container)
	{
		$optCount = 0;
		$num = -1;
		$res = array();
...
class ServiceDefinition extends Nette\Object
{
	...
	function setClass($class, array $args = array())
	{
	...
	function setFactory($factory, array $args = array())
	{
		$this->factory = new Statement($factory, $args);
		return $this;
	}

	function addSetup($target, $args = NULL)
	{
	...
	function addTag($tag, $attrs = TRUE)
	{
		$this->tags[$tag] = $attrs;
		return $this;
	}

	function setAutowired($on)
	{
		$this->autowired = $on;
		return $this;
	}
...
if (class_exists('PDO')){ class Statement extends Nette\Object
{
	...
	function __construct($entity, $arguments)
	{
...
use Nette\Utils\Strings;use Nette\Utils\Validators;
...
class Session extends Nette\Object
{
	...
	function setCookieParameters($path, $domain = NULL, $secure = NULL)
	{
		return $this->setOptions(array(
			'cookie_path' => $path,
...
class FormMacros extends MacroSet
{
	...
	static function renderFormBegin($form, $attrs)
	{
	...
	static function renderFormEnd($form)
	{
		$s = '';
		if (strcasecmp($form->getMethod(), 'get') === 0) {
...
class ClassType extends Nette\Object
{
	...
	function addProperty($name, $value = NULL)
	{
...
class Helpers
{
	...
	static function format($statement)
	{
		$args = func_get_args();
		return self::formatArgs(array_shift($args), $args);
	}

	static function formatArgs($statement, array $args)
	{
		$a = strpos($statement, '?');
		while ($a !== FALSE) {
			if (!$args) {
	...
	static function formatMember($name)
	{
	...
	static function createObject($class, array $props)
	{
...
class Method extends Nette\Object
{
	...
	function addParameter($name, $defaultValue = NULL)
	{
		$param = new Parameter;
		if (func_num_args() > 1) {
	...
	function addBody($statement, array $args = NULL)
	{
...
class Strings
{
	...
	static function substring($s, $start, $length = NULL)
	{
		if ($length === NULL) {
			$length = PHP_INT_MAX;
		}
...
class Validators extends Nette\Object
{
	...
	static function assertField($arr, $field, $expected = NULL)
	{
	...
	static function isUnicode($value)
	{
		return is_string($value) && preg_match('##u', $value);
	}

	static function isNone($value)
	{
		return $value == NULL;
	}
	...
	static function isInRange($value, $range)
	{
